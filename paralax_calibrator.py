...
class ParalaxCalibrator:
    ...
    def __init__(self):
        ...
        self.trml_distortion_center = (
            self.trml_img_size[1] / 2 + self.trml_distortion_center_shift[1],
    ...
    def get_trml_angular_mapping_coef(self, trml1_proj_points, trml_distortion_center):
        ...
        mat_bottom = np.array(
            [
                [p_bottom**2, p_bottom, 1],
                [k_bottom**2, k_bottom, 1],
                [z_bottom**2, z_bottom, 1],
            ]
        )
    ...
    def get_trml_aligned_coordinated_from_angular_mapping(
        self,
        angular_coordinate,
        corner_max_val,
        a_coefs_vector,
...
def cal_device(dev_id):
    ...
    try:
        maskMartix, coordinateMap, sensitivityMatrix = myCal(
            trml_img_path,
            rgb_img_path,
    ...
    np.save(
        outputDir + "/" + dev_id + "/mapped_mask_matrix_" + unit_type + dev_id + ".npy",
        maskMartix,
    )
    np.save(

...
class ParalaxCalibrator:
    ...
    def __init__(self):
        ...
        self.rgb_distortion_center = (self.rgb_img_size[1] / 2, self.rgb_img_size[
            0] / 2)  # (1136,934) #(1178,908) # Calculated using Matlab's script. Relevant for all rgb images
        self.rgb_c = -0.38
        ...
        self.trml_distortion_center = (self.trml_img_size[1] / 2 + self.trml_distortion_center_shift[1],
                                       self.trml_img_size[0] / 2 + self.trml_distortion_center_shift[0])
    ...
    def __call__(self, thermalImageLocation, colorImageLocation, thermalPoints, rgbPoints):
        ...
        :param thermalImageLocation: the file location of the thermal image used (must be 240x320)
        :param colorImageLocation: the file location of the color image used (must be 240x320)
        ...
        assert self.thermalImage.ndim == 2 or self.thermalImage.ndim == 3
        # In case of set of thermal shots, mean their pixels (along 'time' axe)
        if self.thermalImage.ndim == 3:
            self.thermalImage = np.mean(self.thermalImage, axis=0)
        self.thermalImage = np.transpose(self.thermalImage)
        ...
        # Read thermal image
        self.thermalImage = mpimg.imread('trml_matrix_scaled.jpg')
            ...
            for col in range(int((self.rgb_img_size[1] + 2 * self.rgb_padding[1]) / mapping_resolution)):
                ...
                if loop_undistorted_rgb_coordinate is not None:
                    ...
                    if calc_valid_flag:
                        ...
                        if 0 <= loop_trml_coordinate_unbounded[0] < self.trml_img_size[1] and 0 <= \
                                loop_trml_coordinate_unbounded[1] < self.trml_img_size[0]:
    ...
    def calculate_undistorted_coordinates_of_array(self, original_coordinates_array, distortion_center, C, D,
                                                   radius_norm_factor):
        """
    ...
    def get_trml_angular_mapping_coef(self, trml1_proj_points, trml_distortion_center):
        ...
        mat_bottom = np.array(
            [[p_bottom ** 2, p_bottom, 1], [k_bottom ** 2, k_bottom, 1], [z_bottom ** 2, z_bottom, 1]])
    ...
    def get_trml_aligned_coordinated_from_angular_mapping(self, angular_coordinate, corner_max_val, a_coefs_vector,
                                                          b_coefs_vector, c_coefs_vector, distortion_center):
...
def cal_device(row):
    ...
    # Create Cal Files
    # -----------------------------------------------------------
    myCal = ParalaxCalibrator()
    try:
        maskMartix, coordinateMap, sensitivityMatrix = myCal(trml_img_path, rgb_img_path,
                                                             trml_elements_coordinates,
                                                             rgb_elements_coordinates)
    except AttributeError:
        logging.warn("Points not sampled. Skipping device")
        return
...
def get_scaled_trml_image_optimized(trml_arr, scale=10):
    dest_size = (240, 320)
    resized_trml_arr = cv2.resize(trml_arr, dest_size, interpolation=cv2.INTER_CUBIC)
    return resized_trml_arr

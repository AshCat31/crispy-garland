...
class TestDM(unittest.TestCase):
    def test_mainloop_state_transitions(self):
        dm = DM()
        with patch.object(dm, 'state_handlers', {
            'state1': lambda: (self.assertEqual(dm.state, 'state1'), setattr(dm, 'state', 'state2')),
            'state2': lambda: (self.assertEqual(dm.state, 'state2'), setattr(dm, 'state', 'state3')),
            'state3': lambda: (self.assertEqual(dm.state, 'state3'), setattr(dm, 'state', 'state4'))
        }):

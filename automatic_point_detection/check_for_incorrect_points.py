...
def check_if_image_is_shifted(device_id: str, threshold: int, debug: bool, save_directory: str = ''):
    ...
    are_points_over_threshold = (
        max_x >= (240 - threshold) or max_y >= (320-threshold))
...
def check_if_image_is_rotated(device_id: str, threshold: int,  debug: bool, save_directory: str = ''):
    ...
    if save_directory or debug:
        for x, y in calibration_points:
            ...
            cv2.line(thermal_image, (cp[1][0], cp[1][1]),
                     (cp[7][0], cp[7][1]), (0, 255, 0),  2)
            cv2.line(thermal_image, (cp[3][0], cp[3][1]),
                     (cp[5][0], cp[5][1]), (0, 255, 0), 2)
...
def main():
    ...
    parser.add_argument("--threshold", required=False,
                        type=int, default=IGNORE_VALUE, help=THRESHOLD_HELP)
    ...
    if args.device_id:
        func(device_id=args.device_id, threshold=args.threshold,
             save_directory=directory_path, debug=True)
    else:
        calibrated_devices = try_to_read_list_from_file(args.file)

        for device_id in calibrated_devices:
            if func(device_id=device_id, threshold=args.threshold, save_directory=directory_path, debug=args.debug):
                break
